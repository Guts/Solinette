...
#-------------------------------------------------------------------------------
# Name :        Solinette
# Purpose :     Localización automatizada de datos con dirección (Lima y Callao)
#
# Authors :     Pierre Vernier
#
# Created :     19/07/2010
# Updated :     18/07/2012
#
...
import psycopg2     # herramientas de gestión de base de datos PostgreSQL
...
def sentido_n_nm1(inicio_n, final_n, inicio_nm1, final_nm1):
    """Establecer si el sentido es correcto
...
def sentido_n_np1(inicio_n, final_n, inicio_np1, final_np1):
    """Función para establecer si el sentido es correcto entre
...
def entre_puntos(cadena, tuplo):
    """Función para encontrar los 2 puntos de la cuadra entre los cuales se
...
conn = psycopg2.connect('host=10.0.6.46 port=5432 dbname=geolocalizacion user=postgres password=python')
curs = conn.cursor()


# encoding, muy importante
cons_encoding = "SET CLIENT_ENCODING TO 'LATIN1';" # 'WIN1252'
...
tabla_direcciones = 'empresas_top10000'
...
while i < len(lista2):
    ...
    while j < len(lista2[i]) :
        ...
        for comple in lista_comple:
            ...
            # un test car bug avec la condition du haut
            if len(lista2[i]) > 0 and lista2[i][j].find(comple) == 0 and j <> -1: # 0 significa que la sub cadena se encuentra al incio de la cadena de referencia
...
print "Localización de las direcciones"

from math import atan2, pi, sqrt
import random


conn2 = psycopg2.connect('host=10.0.6.46 port=5432 dbname=geolocalizacion user=postgres password=python')
curs2 = conn2.cursor()
    ...
    if cuadra <> '' and lista_dir[1] <> '' and lista_dir[1] <> None and lista_dir[3] <> '': # si tengo una cuadra y un nombre puedo localizar el punto
        ...
        if len(lista_via) > 0:
            ...
            if len(lista_via) > 1: # en el caso donde tengo mas de una via
                ...
                # Abajo para extraer las coordenadas start y end de la cuadra en formato float. No se si lo necesitare
